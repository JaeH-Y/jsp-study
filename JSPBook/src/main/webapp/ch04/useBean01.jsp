...
<body>
	<!-- 자바빈(JavaBean)
			데이터 표현을 목저으로 하는 자바 클래스로
			데이터 전달용 객체(VO, DTO)를 JSP에서 다를 때 주로 사용
			JSP에서는 디자인(HTML)과 비즈니스 로직(Java)를 분리하기 위해 이 자바빈을
			데이터 바구니(DTO/VO)처럼 활용한다.	
	 -->	
	 <!-- useBean 액션 태그에 Date 클래스를 사용하여 현재 날짜와 시각 출력하기 -->
	 <jsp:useBean id="dateBean" class="java.util.Date" />
