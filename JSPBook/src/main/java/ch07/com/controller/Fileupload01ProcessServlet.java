...
public class Fileupload01ProcessServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1) 내부 폴더(톰캣 서버 내 프로젝트) (보통 학습용 or 테스트용)
...
//		String uploadPath = getServletContext().getRealPath("/upload");
		...
		String contentType = filePart.getContentType();	// MIME 타입(이미지, 텍스트, 영상 등등)
		long fileSize = filePart.getSize();	// 파일 크기(byte)
		
		// 3. 파일 저장
//		filePart.write(uploadPath + "/" + fileName);
		filePart.write(uploadPath + File.separator + fileName);	// OS에 맞는 경로를 안전하게 생성(/ 또는 \)
